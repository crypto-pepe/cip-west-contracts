...
import { getIntegerContractValue, invoke } from './transaction';
...
(async () => {
  ...
  switch (options.network) {
    ...
    case 'custom':
      ...
      migrationContract = getEnvironmentByName(
        options.network
            ...
            return new Promise<void>((resolve) => {
              ...
              require(path.resolve(process.cwd(), migrationsDir, migration))
                ...
                .then(() => {
                  ...
                  invoke(
                    {
                      contractId: migrationContract,
                      contractVersion: 1,
                      callFunction: 'set_completed',
                      callPayments: [],
                      callParams: [
                        {
                          type: 'integer',
...
async function deployMigrationScript(
  ...
  // eslint-disable-next-line
  let result = await require(path.resolve(
    process.cwd(),
    migrationsDir,
    migrations[0]
  ))
...
async function getCompletedMigration(
  ...
  const lastMigration = await getIntegerContractValue(
    migrationContract,
